...
> ADR-060 修订实际换武器后的发射起点：元素派生造成 WeaponId 改变时，活动槽位从新周期第 0 帧立即发射，不再等待完整间隔。ADR-061 进一步为三种双元素精确组合增加命中派生效果，替代本文“没有额外命中机制”的实现基线。ADR-072 又将 WeaponId 9 改为独立五秒阶段，到期统一清空三元素并恢复普通法杖，不再按原元素计时自然逐级降级。ADR-073 为武器箱增加法杖、弓、弹弓家族优先级，不限制本文的元素法杖派生与降级。
