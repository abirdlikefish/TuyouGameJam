...
- 子弹伤害上下文使用现有的 `BulletId`、`WeaponId`、`ElementId`、最终伤害和命中位置/方向，不新增 `DamageType`。
...
- 敌人身体、攻击碰撞体、目标检测器和子弹碰撞体必须使用明确的碰撞层。移动由自定义时间驱动；使用 Collider2D 查询时按 `../03_SharedContracts/CollisionRules.md` 在查询阶段显式同步 Physics2D Transform。
