...
- 每次生成请求携带当前 `LevelRunId`、配置 ID、生成点编号和初始世界位置。
- SpawnManager 在 `StopRun` 后不再消费未来条目；每次新会话调用一次 `StartRun(LevelConfig, LevelRunId)`，同时绑定本关配置、切换会话并将三个游标归零。不再暴露语义重复的 `ResetRun`。
...
→ ObstacleManager.Spawn(ObstacleSpawnRequest)
