...
Accepted（正负门命令由 ADR-035 修订；Gate 配置与伤害结算由 ADR-038 修订；失败后 HP 下限由 ADR-046 修订）

> ADR-035 已将非负门改为 `AddArmy`、负数门改为 `RemoveArmy(Abs(GateValue))` 的 Army 内部伤害结算。ADR-038 进一步移除 `TbGate`、`HitIncrement` 与固定 `ElementDuration`，改为 LevelConfig 内联逐门配置、加法门按实际伤害增长，以及元素门用 HP 归零后的额外伤害换算持续时间。ADR-046 又把 Failed 元素门和 Prop 的后续 HP 下限固定为 `1`，不再允许失败后被击破。下文旧字段仅保留决策演进背景；一次接触状态机、正负门命令、失败奖励锁定和命令先于事实事件结论继续有效。
...
- 一旦元素门接触失败，后续子弹仍正常命中并消费，但 HP 最低锁在 `1`，不再增加可兑换额外伤害，也不发放元素奖励或因伤害回收。
- 未接触直接离场不算成功，也不算失败接触。
...
- 道具接触失败后，后续子弹仍正常命中并消费，但 HP 最低锁在 `1`，不发布 `PropBroken`，也不发放任何击破效果；当前 MVP 包括 `WeaponId` 奖励。
- 失败状态锁定全部击破效果；对象仍可产生受击表现，最终只按失败离场或 `StopRun` 清理规则回收。
