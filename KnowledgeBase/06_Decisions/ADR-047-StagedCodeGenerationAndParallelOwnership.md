...
6. `ImplementationPlan.md` 的批次是唯一执行顺序；`Roadmap.md` 的阶段只按能力和验收范围分组，不表达实现先后。一个批次可以推进多个路线图阶段，一个路线图阶段也可以跨多个批次完成。
...
- 路线图继续适合作为能力完成度清单，但不能从阶段编号推导编码顺序；排期和依赖判断统一查看实施计划。
...
- Roadmap 明确阶段仅为验收分组，并将具体代码生成顺序交给 `ImplementationPlan.md`。
