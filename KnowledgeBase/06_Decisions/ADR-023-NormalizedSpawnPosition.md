...
- 移除三路生成点和生成点 ID。敌人、Gate、Prop 共用一条由 `LevelConfig.spawnY` 定义的固定出生横线；左右边界按 ADR-052 从原点居中的道路宽度派生。
...
- `RoadLayoutSnapshot` 保留 `SpawnY`、`LeftBoundary` 和 `RightBoundary`，移除 `LaneSpawnPoints`；后续 ADR-052 固定从道路宽高派生 BottomBoundary、TopBoundary、Width 和 Height，并加入 ArmySpawnPosition。
