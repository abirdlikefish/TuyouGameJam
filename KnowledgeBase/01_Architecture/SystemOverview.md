...
旁路扩展模块：本地关卡进度（已启用）和设置（Deferred）
关卡进度通过公开 `IPlayerProgressStore` 由 Composition 注入，核心层不依赖具体 JSON 实现。
```
...
- 只读 Gameplay HUD 通过 `IGameplayHudSource` 查询权威快照，结果面板监听胜负事实；二者不参与核心数值计算。Gameplay 拖拽控件属于 Input Adapter 的采集端，通过 `IHorizontalInputReceiver` 类型化同步命令提交唯一横向倍率，但不得直接修改玩法对象字段。音频后续接入时遵守只响应事实的约束。
- 本地关卡进度是旁路扩展，不是表现层的下游层；ADR-062 已确定其数据契约和生命周期。设置持久化仍未启用。
- 关卡道路宽高、Army 出生坐标、固定出生/接近/离场线和按时间轴的生成编排放入 `LevelConfig` ScriptableObject；道路固定以世界原点为中心，四边从半宽和半高派生，ArmyRoot 每局从配置坐标开始，道路不设置玩法 Collider；每条生成项使用 `[0,1]` 的归一化横向出生位置。
