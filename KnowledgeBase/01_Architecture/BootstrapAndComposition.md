...
- 绑定并校验 GlobalRoot 下唯一的 `AppCamera` 与同对象 `AudioListener`，使其随应用生命周期持续清屏和渲染当前应用场景。
...
- AppCamera 是应用级渲染资源，不是服务；由 GlobalBootstrap 使用 Inspector 引用校验所有权，不提供静态访问或 Service Locator。
...
└── 校验 AppCamera、场景标识、Build Settings、LevelCatalog、资源注册表和其他序列化引用
...
- 全流程只存在一个启用的 Camera 和一个启用的 AudioListener；二者都属于 GlobalRoot 下同一个 AppCamera。
...
- GameStateService 可以注入假的 Config、Scene 和 EventBus 实现进行纯流程测试，不依赖静态全局状态。
