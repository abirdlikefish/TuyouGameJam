...
- 状态：`InProgress`（39 个 Clip、5 个 Controller、6 个 OverrideController、7 个动画 Prefab 与三类 Monster AnimationEvent 已完成绑定；当前 12 个正式 Clip 共 554 帧已导入并通过工具及运行态校验，剩余 19 个动作素材、竖屏目标分辨率与 Player 内存验收待完成）
- 决策：[ADR-048](../06_Decisions/ADR-048-AnimationAssetPipelineAndPrefabBindings.md)、[ADR-049](../06_Decisions/ADR-049-ContinuousArmyCombatAnimationAndVictoryPresentation.md)
...
- `W000/W001/W002` 与当前 `WeaponId 0/1/2` 对应。
- `Bullet_000/001/002` 与当前 `BulletId 0/1/2` 对应。
...
`Assets/Scripts/Tools/Editor/SequenceAnimationBuilder.cs` 登记当前 31 个正式动作，提供以下 Unity 菜单：
...
截至 2026-09-20，工具已同步 W000/W001/W002 Attack、Normal/Elite/Boss 的 Move 与 Attack，以及 BulletId 0/1/2 Loop；共 12 个正式 Clip、554 帧。Army 其他动作、三种 Monster Death 和四种 Gate Loop 仍保持空轨道。
...
Clip 的 Samples 使用源帧率。当前已分析素材为 24 FPS、51 帧、声明总时长 2.125 秒；生成后必须在 Animation 窗口核对最后一帧确实持有一个采样间隔，而不是只把最后一个关键帧放在 Clip 结束瞬间。

当前动画骨架统一使用 24 FPS。31 个正式 Clip 已预建一个时间 `0`、值为 `None` 的 `SpriteRenderer.Sprite` 轨道：Army 的绑定路径为空字符串，因为 Animator 与 SpriteRenderer 同在 `SoldierVisual`；Monster、Bullet、Gate 的绑定路径为 `Visual`。8 个基础占位 Clip 不含任何曲线，只作为 OverrideController 的稳定替换键，不拖入正式图片。
...
`PF_Army_000` 保存唯一的 `WeaponId -> AnimatorOverrideController` 数组；每个 `ArmySlotView` 显式绑定本槽位 `soldierAnimator`。初始 `WeaponId=0` 和每次实际换武器时，ArmyController 把选中的 Controller 应用到所有槽位。战斗中换武器后直接重播当前 Attack/MoveLeft/MoveRight 状态，不先回 Idle；隐藏槽位保存当前武器与状态，激活时再从第 0 帧播放。
...
`PF_Bullet` 保持一个规范 Prefab。`AC_Bullet` 以整数参数 `BulletId` 选择 `Bullet_000_Loop`、`Bullet_001_Loop`、`Bullet_002_Loop`。初始化池对象时先设置 ID 和目标状态，再激活对象。
...
- 动画骨架已完成：39 个 Clip、Army/Monster 基础状态机、三个 Army OverrideController、三个 Monster OverrideController、Bullet Controller 和两个 Gate Controller 已建立。
- 当前 12 个正式 Clip 已填入 554 帧并通过工具核对；后续素材继续用相同工具写入剩余 19 个正式 Clip，并核对 Samples、Loop Time、首尾持帧与 Sprite 绑定路径。
...
- Bullet：已增加 Animator 引用、`BulletId 0/1/2` 显式状态选择、激活时从第 0 帧起播和池复用重置。
...
- 分别触发 Army 静止、左右移动、攻击、胜利与三次 WeaponId 切换。
- 分别生成三种怪物并完成移动、攻击判定和死亡回收。
- 分别生成三个 BulletId，确认循环动画、方向、命中和回池。
...
- 扫描结果为 12 个已同步、0 个待更新、19 个空目录、0 个非法动作；已有内容总计 554 帧。
- Controller、AOC、Prefab 必需引用无 Missing；Army 三武器全槽位切换、Monster 三类型事件/死亡回收、Bullet 三 ID、Gate 四身份及归池复借状态重置均通过运行态验证。
- 现有 554 张 Sprite 在 StandaloneWindows64 Editor 中的已加载纹理内存合计约 `156.17 MiB`，单组最高约 `17.20 MiB`；全部已使用 Normal 压缩、关闭 Mipmap/ReadWrite，导入最大尺寸不超过 512。
- 当前结果只证明资源身份、状态机与复用边界正确。Army 的 12 个非 Attack 动作、Monster 三个 Death、四个 Gate Loop 仍为空轨道，不能验收画面连续性；最终 Player 构建内存、9:16 与多分辨率也仍待验证。
