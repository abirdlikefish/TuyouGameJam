...
- 接收由 ConfigService 校验、经 SceneService 和 GameplaySceneEntry 注入的 `LevelConfig`、`LevelId`、`LevelRunId`。
...
- 终局时先切换为 `Completed`，停止输入、未来生成和全部玩法阶段，幂等清理当前会话，再向 GameStateService 提交一次 `LevelCompletion`。
...
- 不直接解析 Prefab。池化规范 Prefab 由对应 Manager 的 Inspector 引用；数值配置由对应 Manager 通过注入的 IConfigService 获取。
...
- `LevelConfig`：已通过 ConfigService 校验的只读关卡资产。
- `LevelId`：必须与 `LevelConfig.levelId` 一致。
...
敌人、Prop 和 Bullet 的生命、速度、伤害等可复用数值来自 Luban，并由对应 Manager 通过 IConfigService 取得。Gate 不使用 TbGate：每条门的类型、加法门的初始数字、元素门的 ElementType 与 MaxHp 来自 LevelConfig；本关元素门统一使用 `elementDurationSecondsPerDamage`；加法门统一速度来自 AdditiveGate Prefab，元素门统一速度和接触伤害来自 ElementGate Prefab。规范 Prefab、Collider、Sprite、Animator 和场景固定引用来自 Unity Inspector 或 Unity 资源注册表。
...
├── LevelConfig.cs
├── LevelSpawnEntries.cs
├── LevelManager.cs
└── RoadView.cs
```

- `LevelConfig.cs`：ScriptableObject，只保存关卡编排与道路数值边界。
- `LevelSpawnEntries.cs`：分别定义可序列化的 Enemy、Gate、Prop 生成项；避免使用 Unity 难以稳定序列化的泛型基类。
- `LevelManager.cs`：MonoBehaviour，实现 ILevelRuntime 和本局协调逻辑。
- `RoadView.cs`：根据 RoadLayoutSnapshot 调整道路视觉，不持有玩法状态、不添加道路 Collider。

公共接口和 DTO 按职责放入 `Assets/Scripts/Game/Contracts`；`GameplaySceneEntry.cs` 属于 Composition；LevelConfig 的跨表验证器属于 Config 模块，不放入 LevelManager。
...
GameplaySceneEntry 通过 Inspector 持有 LevelManager、SpawnManager、ArmyController、BulletManager、EnemyManager、ObstacleManager、GameplayInputAdapter、TouchDragInput、Gameplay Canvas、GraphicRaycaster、EventSystem、StandaloneInputModule、RoadView 和相应 Root。Entry 取得应用服务后只把每个消费者需要的最小接口注入，不使用 Find、运行时 AddComponent、静态服务定位器或缺失引用兜底。
...
```text
GameplaySceneEntry 注入 LevelConfig、LevelId、LevelRunId 和接口
...
任一步失败时按逆序执行已完成部分的幂等 StopRun，并让 GameplaySceneEntry 返回初始化失败；不得发布 Ready 或 LevelRunStarted。
...
GameStateService 保留本次已校验 LevelConfig 的结果数据；Victory 的 `unlockedLevelIds` 由它防御性复制并发布，不加入 LevelCompletion。

## 配置验证

ConfigService 在进入 Gameplay 前验证：
...
- 初始化任一步失败时不发布 AppSceneReady(Gameplay)，并逆序清理已经启动的模块。

## 已知问题与进入代码前仍需解决

- Layer Collision Matrix 已由 ADR-037 定案为默认关闭自动物理关系；工程实现时仍需在目标 Unity 版本验证显式 Cast/Overlap 按 ContactFilter2D/LayerMask 返回指定目标。
- `DES-042`：IConfigService 尚未补齐 Bullet、Enemy、Prop 的类型化只读查询、快照和缓存边界；Army/Weapon 已由 ADR-035 定案，Gate 已由 ADR-038 明确不读表。需要配置表的 Manager 不能直接访问静态 Luban Tables。
- `DES-043`：Animator 攻击判定帧如何稳定登记并在 EnemyManager.ResolveAttacks 中消费尚未定案，不能让 Animator 回调直接造成伤害。
- `DES-044`：Manager `StartRun/StopRun` 当前返回 void，初始化失败、运行时阶段异常、稳定错误来源和逆序回滚的公共语义仍需确认。
