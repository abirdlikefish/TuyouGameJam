...
每个激活槽位按相同的 `TbWeapon.FireInterval` 独立触发射击。弹弓与弓每次生成一枚竖直向上的基础子弹；普通法杖和七种元素法杖每次从同一 FirePoint 同时生成三枚，方向固定为竖直 `(0,1)`、上偏左 `(-3,13).normalized`、上偏右 `(3,13).normalized`。方向只表示轨迹，三颗子弹仍分别使用对应 `TbBullet.MoveSpeed`。`RepresentedCount` 不缩放射速、伤害或弹丸数量，也不将逻辑人数直接转换为等量子弹。
...
- 法杖三颗子弹从同一 FirePoint 同帧生成，方向分别为 `(0,1)`、`(-3,13).normalized`、`(3,13).normalized`，速度大小都保持对应 Bullet 配置值。
