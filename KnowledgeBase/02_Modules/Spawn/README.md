...
- 在 LevelManager 进入终局后停止消费未来生成项，并在新会话开始时重置三个游标。Victory 不等待 Gate/Prop 游标完成，未来道路对象条目会被有意截断。
...
Enemy/Prop 条目另有对应 `TbEnemy`/`TbProp` 的非负 `configId`，且 `0` 合法；Prop 的 `configId` 可选择武器箱、篮球或鹅笼。Gate 条目没有配置 ID。Enemy、Gate、Prop 三类时间轴请求都携带对应列表的 `SpawnEntryIndex`；ikun 动态篮球另使用关卡顶层 `ikunBasketballConfigId`，不占用时间轴游标。
...
- 终局后不再生成尚未到时的对象；新会话调用一次 `StartRun(LevelConfigSnapshot, RoadLayoutSnapshot, LevelRunId)`，绑定本关不可变快照与道路快照并将所有游标归零。
- Victory 仍只要求敌人时间轴完成且存活敌人为零；Gate/Prop 游标可以尚未到末尾，StopRun 直接停止它们，不补发剩余条目。
...
- `SpawnY` 与后续 `EnemyApproachY`、`DespawnY` 判定都使用实例根 GameObject 中心，不使用 Collider/Renderer 边缘。
