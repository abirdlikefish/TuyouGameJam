...
- 决策：`../../06_Decisions/ADR-005-MonsterCombatAndManager.md`、`../../06_Decisions/ADR-031-TypedComponentPoolsAndDefensiveDeactivation.md`
...
子弹碰撞向敌人传入 `BulletDamageContext`，包含 `BulletId`、`WeaponId`、`ElementId`、最终伤害、命中位置和方向。Monster 通过 Manager 注入的发布回调报告 `MonsterDamaged`；生命值归零时先调用 EnemyManager 的必执行死亡回调，由 EnemyManager 完成死亡去重和 `AliveEnemyCount` 更新，再发布携带最后一击上下文的 `MonsterKilled`。MVP 的反馈表现由 Animator、VFX 或其他视觉适配器消费，不在 Monster 内写死具体资源；声音功能延后。
